...
//Actor constants
static const int32_t FACTOR = 5;

//Actor state
typedef struct {
  AbstractActorInstance base;
  int _fsmState;
} ActorInstance_Example_m3;

//Actor functions
ART_ACTION_CONTEXT(1, 1)

ART_ACTION(action0, ActorInstance_Example_m3);
ART_ACTION_SCHEDULER(Example_m3_action_scheduler);
static void ActorInstance_Example_m3_constructor(AbstractActorInstance *);
...
ActorClass klass = INIT_ActorClass(
  "Example_m3",
  ActorInstance_Example_m3,
  ActorInstance_Example_m3_constructor,
  0, //setParam not needed anymore (we instantiate with params)
  Example_m3_action_scheduler,
  0, // no destructor
...
#define ActorInstance_Example_m3__defaultState_ID 0
static void ActorInstance_Example_m3_constructor(AbstractActorInstance *pBase) {
  ActorInstance_Example_m3 *thisActor=(ActorInstance_Example_m3*) pBase;
  thisActor->_fsmState = ActorInstance_Example_m3__defaultState_ID;//Initial state
  {
  }
...
ART_ACTION_SCHEDULER(Example_m3_action_scheduler) {
  ...
  while(1) {
    ...
    switch(thisActor->_fsmState) {
      case ActorInstance_Example_m3__defaultState_ID:
        result = exitcode_block_Any;
        goto out;
        break;
