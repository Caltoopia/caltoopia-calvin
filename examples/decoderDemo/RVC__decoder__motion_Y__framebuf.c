...
static const int32_t SAMPLES_PER_MB = 16;
static const int32_t COMP_PER_MB = 4;
...
} ActorInstance_RVC_decoder__motion_Y__framebuf;

//Actor functions
void RVC_decoder__motion_Y__framebufadvance_block(ActorInstance_RVC_decoder__motion_Y__framebuf* thisActor) {
  thisActor->outstanding_writes = (thisActor->outstanding_writes + 1);
  thisActor->comp_read = (thisActor->comp_read + 1);
  if ((thisActor->comp_read == 4)) {
...
ART_ACTION(action0__cmd__newVop, ActorInstance_RVC_decoder__motion_Y__framebuf);
ART_ACTION(action1__read_w_h, ActorInstance_RVC_decoder__motion_Y__framebuf);
ART_ACTION(action2__cmd__noMotion, ActorInstance_RVC_decoder__motion_Y__framebuf);
ART_ACTION(action3__cmd__motion, ActorInstance_RVC_decoder__motion_Y__framebuf);
ART_ACTION(action4__read_mv, ActorInstance_RVC_decoder__motion_Y__framebuf);
ART_ACTION(action5__cmd__neither, ActorInstance_RVC_decoder__motion_Y__framebuf);
ART_ACTION(action6__read_fb, ActorInstance_RVC_decoder__motion_Y__framebuf);
ART_ACTION(action7, ActorInstance_RVC_decoder__motion_Y__framebuf);
ART_ACTION_SCHEDULER(RVC_decoder__motion_Y__framebuf_action_scheduler);
static void ActorInstance_RVC_decoder__motion_Y__framebuf_constructor(AbstractActorInstance *);
...
ActorClass klass=INIT_ActorClass(
  "RVC_decoder__motion_Y__framebuf",
  ActorInstance_RVC_decoder__motion_Y__framebuf,
  ActorInstance_RVC_decoder__motion_Y__framebuf_constructor,
  0, //setParam not needed anymore (we instantiate with params)
  0, 0, // serialize/deserialize
  RVC_decoder__motion_Y__framebuf_action_scheduler,
  0, // no destructor
...
bool_t RVC_decoder__motion_Y__framebuf__DOLLAR__d7382(art_action_context_t *context, ActorInstance_RVC_decoder__motion_Y__framebuf *thisActor) {
  int32_t cmd;
  {
    bool_t ret;
...
bool_t RVC_decoder__motion_Y__framebuf__DOLLAR__d7406(art_action_context_t *context, ActorInstance_RVC_decoder__motion_Y__framebuf *thisActor) {
  {
    bool_t ret;
    ret = ((thisActor->outstanding_writes < ((thisActor->vop_height_read * thisActor->vop_width_read) * 4)) || (thisActor->vop_height_read == 0));
    return ret;
  }
}
...
ART_ACTION(action1__read_w_h, ActorInstance_RVC_decoder__motion_Y__framebuf) {
  ...
  last_frame_size = ((thisActor->vop_height_read * 16) * 1936);
  thisActor->vop_width_read = w;
  thisActor->vop_height_read = h;
  thisActor->next_vop_width_write = w;
  thisActor->next_vop_height_write = h;
  thisActor->vop_width_in_samples = (w * 16);
  thisActor->vop_height_in_samples = (h * 16);
  thisActor->max_outstanding_writes = (((thisActor->vop_height_read - 2) * thisActor->vop_width_read) * 4);
  thisActor->mbx_read = 0;
...
bool_t RVC_decoder__motion_Y__framebuf__DOLLAR__d7459(art_action_context_t *context, ActorInstance_RVC_decoder__motion_Y__framebuf *thisActor) {
  int32_t cmd;
  {
    bool_t ret;
    cmd = pinPeek_int32_t(IN1_BTYPE, 0);
...
ART_ACTION(action2__cmd__noMotion, ActorInstance_RVC_decoder__motion_Y__framebuf) {
  ...
  cmd = pinRead_int32_t(IN1_BTYPE);
  RVC_decoder__motion_Y__framebufadvance_block(thisActor);
  ART_ACTION_EXIT(action2__cmd__noMotion, 2);
}

bool_t RVC_decoder__motion_Y__framebuf__DOLLAR__d7471(art_action_context_t *context, ActorInstance_RVC_decoder__motion_Y__framebuf *thisActor) {
  int32_t cmd;
  {
    bool_t ret;
    cmd = pinPeek_int32_t(IN1_BTYPE, 0);
...
bool_t RVC_decoder__motion_Y__framebuf__DOLLAR__d7544(art_action_context_t *context, ActorInstance_RVC_decoder__motion_Y__framebuf *thisActor) {
  {
    bool_t ret;
...
ART_ACTION(action6__read_fb, ActorInstance_RVC_decoder__motion_Y__framebuf) {
  ...
  clip_right = 0;
  clip_left = 0;
  y = (((thisActor->mby_read * 16) + (8 * rshift(thisActor->comp_read, 1))) + thisActor->mvy);
  clip_y = (((y < 0)) ? (0):((((y < thisActor->vop_height_in_samples)) ? (y):((thisActor->vop_height_in_samples - 1)))));
  x = (((thisActor->mbx_read * 16) + (8 * bitand(thisActor->comp_read, 1))) + thisActor->mvx);
  clip_x = x;
  {
    int32_t k;
    for(k = 1; k <= 81; k++)
...
bool_t RVC_decoder__motion_Y__framebuf__DOLLAR__d7758(art_action_context_t *context, ActorInstance_RVC_decoder__motion_Y__framebuf *thisActor) {
  {
    bool_t ret;
...
ART_ACTION(action7, ActorInstance_RVC_decoder__motion_Y__framebuf) {
  ...
  if ((thisActor->comp_write == 4)) {
    thisActor->mbx_write = (thisActor->mbx_write + 1);
    thisActor->mb_write_ptr = (thisActor->mb_write_ptr + 16);
    ...
    if ((thisActor->mbx_write == thisActor->vop_width_write)) {
      ...
      thisActor->mb_write_ptr = ((thisActor->mb_write_ptr - (16 * thisActor->vop_width_write)) + 30976);
...
#define ActorInstance_RVC_decoder__motion_Y__framebuf__cmd_ID 0
#define ActorInstance_RVC_decoder__motion_Y__framebuf__read_w_h_ID 1
#define ActorInstance_RVC_decoder__motion_Y__framebuf__read_mv_ID 2
#define ActorInstance_RVC_decoder__motion_Y__framebuf__read_fb_ID 3
static void ActorInstance_RVC_decoder__motion_Y__framebuf_constructor(AbstractActorInstance *pBase) {
  ActorInstance_RVC_decoder__motion_Y__framebuf *thisActor=(ActorInstance_RVC_decoder__motion_Y__framebuf*) pBase;
  thisActor->_fsmState = ActorInstance_RVC_decoder__motion_Y__framebuf__cmd_ID;//Initial state
  {
...
ART_ACTION_SCHEDULER(RVC_decoder__motion_Y__framebuf_action_scheduler) {
  ...
  while(1) {
    if ((pinAvailIn_int32_t(IN2_WD) >= 64)) {
      if (RVC_decoder__motion_Y__framebuf__DOLLAR__d7758(context, thisActor)) {
        ART_FIRE_ACTION(action7);
        continue;
      }
    }
    switch(thisActor->_fsmState) {
      case ActorInstance_RVC_decoder__motion_Y__framebuf__cmd_ID:
        if ((pinAvailIn_int32_t(IN1_BTYPE) >= 1)) {
          if (RVC_decoder__motion_Y__framebuf__DOLLAR__d7382(context, thisActor)) {
            ART_FIRE_ACTION(action0__cmd__newVop);
            thisActor->_fsmState = ActorInstance_RVC_decoder__motion_Y__framebuf__read_w_h_ID;
            continue;
          }
        }
        if ((pinAvailIn_int32_t(IN1_BTYPE) >= 1)) {
          if (RVC_decoder__motion_Y__framebuf__DOLLAR__d7459(context, thisActor)) {
            ART_FIRE_ACTION(action2__cmd__noMotion);
            thisActor->_fsmState = ActorInstance_RVC_decoder__motion_Y__framebuf__cmd_ID;
            continue;
          }
        }
        if ((pinAvailIn_int32_t(IN1_BTYPE) >= 1)) {
          if (RVC_decoder__motion_Y__framebuf__DOLLAR__d7471(context, thisActor)) {
            ART_FIRE_ACTION(action3__cmd__motion);
            thisActor->_fsmState = ActorInstance_RVC_decoder__motion_Y__framebuf__read_mv_ID;
            continue;
          }
        }
        if ((pinAvailIn_int32_t(IN1_BTYPE) >= 1)) {
          if ((pinAvailOut_int32_t(OUT0_halfpel) >= 1)) {
            ...
            thisActor->_fsmState = ActorInstance_RVC_decoder__motion_Y__framebuf__read_fb_ID;
            continue;
      ...
      case ActorInstance_RVC_decoder__motion_Y__framebuf__read_w_h_ID:
        if ((pinAvailIn_int32_t(IN1_BTYPE) >= 1*2)) {
          if (RVC_decoder__motion_Y__framebuf__DOLLAR__d7406(context, thisActor)) {
      ...
      case ActorInstance_RVC_decoder__motion_Y__framebuf__read_mv_ID:
      ...
      case ActorInstance_RVC_decoder__motion_Y__framebuf__read_fb_ID:
        if (RVC_decoder__motion_Y__framebuf__DOLLAR__d7544(context, thisActor)) {
