...
typedef struct {
  ...
  int32_t count;
  int32_t s_quant;
} ActorInstance_RVC_decoder__texture_U__IAP;

//Actor functions
static inline int32_t RVC_decoder__texture_U__IAPdivroundnearest(ActorInstance_RVC_decoder__texture_U__IAP* thisActor, int32_t i, int32_t iDenom){
  return (((i >= 0)) ? (((i + (iDenom >> 1)) / iDenom)):(((i - (iDenom >> 1)) / iDenom)));
}
ART_ACTION_CONTEXT(5, 1)

ART_ACTION(action0__newvop, ActorInstance_RVC_decoder__texture_U__IAP);
ART_ACTION(action1__skip, ActorInstance_RVC_decoder__texture_U__IAP);
ART_ACTION(action2__start, ActorInstance_RVC_decoder__texture_U__IAP);
ART_ACTION(action3__advance, ActorInstance_RVC_decoder__texture_U__IAP);
ART_ACTION(action4__copy, ActorInstance_RVC_decoder__texture_U__IAP);
ART_ACTION_SCHEDULER(RVC_decoder__texture_U__IAP_action_scheduler);
...
ActorClass klass=INIT_ActorClass(
  "RVC_decoder__texture_U__IAP",
  ActorInstance_RVC_decoder__texture_U__IAP,
  ActorInstance_RVC_decoder__texture_U__IAP_constructor,
  0, //setParam not needed anymore (we instantiate with params)
  RVC_decoder__texture_U__IAP_action_scheduler,
  0, // no destructor
...
bool_t RVC_decoder__texture_U__IAP__DOLLAR__d10566(art_action_context_t *context, ActorInstance_RVC_decoder__texture_U__IAP *thisActor) {
  int32_t s;
  {
    bool_t ret;
...
bool_t RVC_decoder__texture_U__IAP__DOLLAR__d10577(art_action_context_t *context, ActorInstance_RVC_decoder__texture_U__IAP *thisActor) {
  int32_t s;
  {
    bool_t ret;
    s = pinPeek_int32_t(IN2_AC_PRED_DIR, 0);
...
bool_t RVC_decoder__texture_U__IAP__DOLLAR__d10606(art_action_context_t *context, ActorInstance_RVC_decoder__texture_U__IAP *thisActor) {
  {
    bool_t ret;
...
ART_ACTION(action4__copy, ActorInstance_RVC_decoder__texture_U__IAP) {
  ...
  if ((thisActor->acpred_flag && ((thisActor->top && top_edge) || ((!(thisActor->top)) && left_edge)))) {
    if ((thisActor->s_prev_quant == thisActor->s_quant)) {
      ...
      pred = (pred + RVC_decoder__texture_U__IAPdivroundnearest(thisActor, (thisActor->buf[bitor(lshift(thisActor->pred_ptr, 4), index)] * thisActor->s_prev_quant), thisActor->s_quant));
    }
  }
...
#define ActorInstance_RVC_decoder__texture_U__IAP__start_ID 0
#define ActorInstance_RVC_decoder__texture_U__IAP__pred_ID 1
static void ActorInstance_RVC_decoder__texture_U__IAP_constructor(AbstractActorInstance *pBase) {
  ActorInstance_RVC_decoder__texture_U__IAP *thisActor=(ActorInstance_RVC_decoder__texture_U__IAP*) pBase;
  thisActor->_fsmState = ActorInstance_RVC_decoder__texture_U__IAP__start_ID;//Initial state
  {
    thisActor->buf = malloc((sizeof(int32_t) * 1968));
    {
      int32_t k;
      for(k = 1; k <= 1968; k++)
      {
...
ART_ACTION_SCHEDULER(RVC_decoder__texture_U__IAP_action_scheduler) {
  ...
  while(1) {
    switch(thisActor->_fsmState) {
      case ActorInstance_RVC_decoder__texture_U__IAP__start_ID:
        if ((pinAvailIn_int32_t(IN2_AC_PRED_DIR) >= 1)) {
          if (RVC_decoder__texture_U__IAP__DOLLAR__d10566(context, thisActor)) {
            ART_FIRE_ACTION(action0__newvop);
            thisActor->_fsmState = ActorInstance_RVC_decoder__texture_U__IAP__start_ID;
            continue;
          }
        }
        if ((pinAvailIn_int32_t(IN2_AC_PRED_DIR) >= 1)) {
          if (RVC_decoder__texture_U__IAP__DOLLAR__d10577(context, thisActor)) {
            ART_FIRE_ACTION(action1__skip);
            thisActor->_fsmState = ActorInstance_RVC_decoder__texture_U__IAP__pred_ID;
            continue;
          }
        }
      ...
      case ActorInstance_RVC_decoder__texture_U__IAP__pred_ID:
        if (RVC_decoder__texture_U__IAP__DOLLAR__d10606(context, thisActor)) {
