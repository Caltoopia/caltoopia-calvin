...
typedef struct {
  ...
  int32_t count;
  int32_t quant;
} ActorInstance_RVC_decoder__texture_Y__IQ;

//Actor functions
static inline int32_t RVC_decoder__texture_Y__IQsaturate(ActorInstance_RVC_decoder__texture_Y__IQ* thisActor, int32_t x){
  bool_t minus;
  bool_t plus;
  int32_t ret;
  minus = (x < (-(2048)));
...
static inline int32_t RVC_decoder__texture_Y__IQabs(ActorInstance_RVC_decoder__texture_Y__IQ* thisActor, int32_t x){
  return (((x < 0)) ? ((-(x))):(x));
}
ART_ACTION_CONTEXT(3, 1)

ART_ACTION(action0__get_qp, ActorInstance_RVC_decoder__texture_Y__IQ);
ART_ACTION(action1__ac, ActorInstance_RVC_decoder__texture_Y__IQ);
ART_ACTION_SCHEDULER(RVC_decoder__texture_Y__IQ_action_scheduler);
static void ActorInstance_RVC_decoder__texture_Y__IQ_constructor(AbstractActorInstance *);
...
ActorClass klass=INIT_ActorClass(
  "RVC_decoder__texture_Y__IQ",
  ActorInstance_RVC_decoder__texture_Y__IQ,
  ActorInstance_RVC_decoder__texture_Y__IQ_constructor,
  0, //setParam not needed anymore (we instantiate with params)
  RVC_decoder__texture_Y__IQ_action_scheduler,
  0, // no destructor
...
ART_ACTION(action1__ac, ActorInstance_RVC_decoder__texture_Y__IQ) {
  ...
  pinReadRepeat_int32_t(IN1_AC, i, 63);
  {
    ...
    for(j = 0; j <= 62; j++)
    {
      v[((0 + j) - 0)] = ((thisActor->quant * (lshift(RVC_decoder__texture_Y__IQabs(thisActor, i[j]), 1) + 1)) - thisActor->round);
    }
  }
  {
    int32_t k;
    for(k = 0; k <= 62; k++)
    {
      o[((0 + k) - 0)] = (((i[k] == 0)) ? (0):((((i[k] < 0)) ? (RVC_decoder__texture_Y__IQsaturate(thisActor, (-(v[k])))):(RVC_decoder__texture_Y__IQsaturate(thisActor, v[k])))));
    }
  }
...
#define ActorInstance_RVC_decoder__texture_Y__IQ__start_ID 0
#define ActorInstance_RVC_decoder__texture_Y__IQ__ac_ID 1
static void ActorInstance_RVC_decoder__texture_Y__IQ_constructor(AbstractActorInstance *pBase) {
  ActorInstance_RVC_decoder__texture_Y__IQ *thisActor=(ActorInstance_RVC_decoder__texture_Y__IQ*) pBase;
  thisActor->_fsmState = ActorInstance_RVC_decoder__texture_Y__IQ__start_ID;//Initial state
  {
  }
...
ART_ACTION_SCHEDULER(RVC_decoder__texture_Y__IQ_action_scheduler) {
  ...
  while(1) {
    switch(thisActor->_fsmState) {
      case ActorInstance_RVC_decoder__texture_Y__IQ__start_ID:
        if ((pinAvailIn_int32_t(IN2_QP) >= 1) && (pinAvailIn_int32_t(IN0_DC) >= 1)) {
          if ((pinAvailOut_int32_t(OUT0_OUT) >= 1)) {
            ...
            thisActor->_fsmState = ActorInstance_RVC_decoder__texture_Y__IQ__ac_ID;
            continue;
      ...
      case ActorInstance_RVC_decoder__texture_Y__IQ__ac_ID:
        if ((pinAvailIn_int32_t(IN1_AC) >= 63)) {
          if ((pinAvailOut_int32_t(OUT0_OUT) >= 63)) {
            ART_FIRE_ACTION(action1__ac);
            thisActor->_fsmState = ActorInstance_RVC_decoder__texture_Y__IQ__start_ID;
            continue;
