...
//Actor functions
inline int32_t RVC_decoder__texture_U__IQsaturate(ActorInstance_RVC_decoder__texture_U__IQ* thisActor, int32_t x){
  bool_t minus;
  bool_t plus;
  int32_t ret;
  minus = (x < (-(2048)));
...
inline int32_t RVC_decoder__texture_U__IQabs(ActorInstance_RVC_decoder__texture_U__IQ* thisActor, int32_t x){
  return (((x < 0)) ? ((-(x))):(x));
}
