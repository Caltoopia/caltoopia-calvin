...
typedef struct {
  ...
  int _fsmState;
} ActorInstance_RVC_decoder__motion_Y__add;

//Actor functions
ART_ACTION_CONTEXT(3, 1)

ART_ACTION(action0__cmd__newVop, ActorInstance_RVC_decoder__motion_Y__add);
ART_ACTION(action1__cmd__textureOnly, ActorInstance_RVC_decoder__motion_Y__add);
ART_ACTION(action2__cmd__motionOnly, ActorInstance_RVC_decoder__motion_Y__add);
ART_ACTION(action3__cmd__other, ActorInstance_RVC_decoder__motion_Y__add);
ART_ACTION(action4__texture, ActorInstance_RVC_decoder__motion_Y__add);
ART_ACTION(action5__motion, ActorInstance_RVC_decoder__motion_Y__add);
ART_ACTION(action6__combine, ActorInstance_RVC_decoder__motion_Y__add);
ART_ACTION_SCHEDULER(RVC_decoder__motion_Y__add_action_scheduler);
static void ActorInstance_RVC_decoder__motion_Y__add_constructor(AbstractActorInstance *);
...
ActorClass klass=INIT_ActorClass(
  "RVC_decoder__motion_Y__add",
  ActorInstance_RVC_decoder__motion_Y__add,
  ActorInstance_RVC_decoder__motion_Y__add_constructor,
  0, //setParam not needed anymore (we instantiate with params)
  RVC_decoder__motion_Y__add_action_scheduler,
  0, // no destructor
...
bool_t RVC_decoder__motion_Y__add__DOLLAR__d1551(art_action_context_t *context, ActorInstance_RVC_decoder__motion_Y__add *thisActor) {
  int32_t cmd;
  {
    bool_t ret;
...
bool_t RVC_decoder__motion_Y__add__DOLLAR__d1563(art_action_context_t *context, ActorInstance_RVC_decoder__motion_Y__add *thisActor) {
  int32_t cmd;
  {
    bool_t ret;
    cmd = pinPeek_int32_t(IN2_BTYPE, 0);
...
bool_t RVC_decoder__motion_Y__add__DOLLAR__d1575(art_action_context_t *context, ActorInstance_RVC_decoder__motion_Y__add *thisActor) {
  int32_t cmd;
  {
    bool_t ret;
    cmd = pinPeek_int32_t(IN2_BTYPE, 0);
...
#define ActorInstance_RVC_decoder__motion_Y__add__cmd_ID 0
#define ActorInstance_RVC_decoder__motion_Y__add__skipw_ID 1
#define ActorInstance_RVC_decoder__motion_Y__add__texture_ID 2
#define ActorInstance_RVC_decoder__motion_Y__add__motion_ID 3
#define ActorInstance_RVC_decoder__motion_Y__add__combine_ID 4
#define ActorInstance_RVC_decoder__motion_Y__add__skiph_ID 5
static void ActorInstance_RVC_decoder__motion_Y__add_constructor(AbstractActorInstance *pBase) {
  ActorInstance_RVC_decoder__motion_Y__add *thisActor=(ActorInstance_RVC_decoder__motion_Y__add*) pBase;
  thisActor->_fsmState = ActorInstance_RVC_decoder__motion_Y__add__cmd_ID;//Initial state
  {
  }
...
ART_ACTION_SCHEDULER(RVC_decoder__motion_Y__add_action_scheduler) {
  ...
  while(1) {
    switch(thisActor->_fsmState) {
      case ActorInstance_RVC_decoder__motion_Y__add__cmd_ID:
        if ((pinAvailIn_int32_t(IN2_BTYPE) >= 1)) {
          if (RVC_decoder__motion_Y__add__DOLLAR__d1551(context, thisActor)) {
            ART_FIRE_ACTION(action0__cmd__newVop);
            thisActor->_fsmState = ActorInstance_RVC_decoder__motion_Y__add__skipw_ID;
            continue;
          }
        }
        if ((pinAvailIn_int32_t(IN2_BTYPE) >= 1)) {
          if (RVC_decoder__motion_Y__add__DOLLAR__d1563(context, thisActor)) {
            ART_FIRE_ACTION(action1__cmd__textureOnly);
            thisActor->_fsmState = ActorInstance_RVC_decoder__motion_Y__add__texture_ID;
            continue;
          }
        }
        if ((pinAvailIn_int32_t(IN2_BTYPE) >= 1)) {
          if (RVC_decoder__motion_Y__add__DOLLAR__d1575(context, thisActor)) {
            ART_FIRE_ACTION(action2__cmd__motionOnly);
            thisActor->_fsmState = ActorInstance_RVC_decoder__motion_Y__add__motion_ID;
            continue;
          }
        }
        if ((pinAvailIn_int32_t(IN2_BTYPE) >= 1)) {
          ART_FIRE_ACTION(action3__cmd__other);
          thisActor->_fsmState = ActorInstance_RVC_decoder__motion_Y__add__combine_ID;
          continue;
        }
      ...
      case ActorInstance_RVC_decoder__motion_Y__add__skipw_ID:
        if ((pinAvailIn_int32_t(IN2_BTYPE) >= 1)) {
          ART_FIRE_ACTION(action3__cmd__other);
          thisActor->_fsmState = ActorInstance_RVC_decoder__motion_Y__add__skiph_ID;
          continue;
        }
        result = exitcode_block_Any;
        goto out;
      ...
      case ActorInstance_RVC_decoder__motion_Y__add__texture_ID:
        if ((pinAvailIn_int32_t(IN1_TEX) >= 64)) {
          if ((pinAvailOut_int32_t(OUT0_VID) >= 64)) {
            ART_FIRE_ACTION(action4__texture);
            thisActor->_fsmState = ActorInstance_RVC_decoder__motion_Y__add__cmd_ID;
            continue;
      ...
      case ActorInstance_RVC_decoder__motion_Y__add__motion_ID:
      ...
      case ActorInstance_RVC_decoder__motion_Y__add__combine_ID:
      ...
      case ActorInstance_RVC_decoder__motion_Y__add__skiph_ID:
        if ((pinAvailIn_int32_t(IN2_BTYPE) >= 1)) {
          ART_FIRE_ACTION(action3__cmd__other);
          thisActor->_fsmState = ActorInstance_RVC_decoder__motion_Y__add__cmd_ID;
          continue;
